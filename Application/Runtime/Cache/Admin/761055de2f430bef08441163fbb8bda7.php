...
<body>
	...
	<div class="right_box">
		<form action="/index.php/Admin/Article/update/id/<?php echo ($item["id"]); ?>" method="post">
		<input type="hidden" name="id" value=<?php echo ($item["id"]); ?>>
		题目：<input type="text" name="title" value=<?php echo ($item["title"]); ?>><br>
		标签：<input type="text" name="tag" value=<?php echo ($item["tag"]); ?>><br>
		分类:
		<select name="category_id">
			<?php if(is_array($result)): foreach($result as $key=>$vo): if(($vo["id"]) == $item["category_id"]): ?><option value =<?php echo ($vo["id"]); ?> selected> <?php echo ($vo["category_name"]); ?></option>
			<?php else: ?>
				<option value =<?php echo ($vo["id"]); ?>> <?php echo ($vo["category_name"]); ?></option><?php endif; endforeach; endif; ?>
 		</select>
 		<br>
		<input type="hidden" name="user_id" value=<?php echo ($item["user_id"]); ?>>
		排序：<input type="text" name="sort" value=<?php echo ($item["sort"]); ?>><br>
		本文内容：
		<!-- 加载编辑器的容器 -->
		<div>
